...
! *** Variables specific for TSMP-PDAF ***
  ...
  ! gw
  ! Vector holding local state vector dimensions for processors of a single model communicator
  ...
  ! Vector holding state-vector indices of grid cells surrounding interpolation for PE-local domain
  INTEGER, ALLOCATABLE :: obs_interp_indices_p(:,:)
  ! Vector holding weights of grid cells surrounding observation for PE-local domain
  INTEGER, ALLOCATABLE :: obs_interp_weights_p(:,:)
  ...
  ! indices of grid cells with smaller lon/lat than observation location
  INTEGER, ALLOCATABLE :: longxy_obs_floor(:), latixy_obs_floor(:)
