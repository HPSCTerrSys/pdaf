...
! !ARGUMENTS:
  ...
  subroutine define_clm_statevec(mype)
    ...
    if(clmupdate_snow==1 .or. clmupdate_snow==2) then
        ...
        do jj=clm_begc,clm_endc
          ...
          if (g == j) then
            if (newgridcell) then
              ...
              if(clmupdate_snow==1) then
                clm_statevec(cc+offset) = snow_depth(jj)
              else if(clmupdate_snow==2) then
                clm_statevec(cc+offset) = h2osno(jj)
              else if(clmupdate_snow==3) then
                clm_statevec(cc+offset) = snow_depth(jj)
                clm_statevec(cc+clm_varsize+offset) = h2osno(jj)
              else if(clmupdate_snow==4) then
                clm_statevec(cc+offset) = snow_depth(jj)
                clm_statevec(cc+clm_varsize+offset) = h2osno(jj)
              else if(clmupdate_snow==5) then
                clm_statevec(cc+offset) = snow_depth(jj)
                clm_statevec(cc+clm_varsize+offset) = h2osno(jj)
              else if(clmupdate_snow==6) then
                clm_statevec(cc+offset) = snow_depth(jj)
                clm_statevec(cc+clm_varsize+offset) = h2osno(jj)
              else if(clmupdate_snow==7) then
                clm_statevec(cc+offset) = snow_depth(jj)
                clm_statevec(cc+clm_varsize+offset) = h2osno(jj)
              else
  ...
  subroutine update_clm(tstartcycle, mype) bind(C,name="update_clm")
    ...
    IF(clmt_printensemble == tstartcycle .OR. clmt_printensemble < 0) THEN
      ...
      IF(clmupdate_swc/=0 .OR. clmupdate_snow/=0) THEN
        ! TSMP-PDAF: For debug runs, output the state vector in files
    ...
    if(clmupdate_snow/=0) then
      ...
      ! Repartitioning
      if ( clmupdate_snow_repartitioning==1 .or. clmupdate_snow_repartitioning==2) then

        if (clmupdate_snow==1) then
          if ( SUM(ABS(snow_depth_in(:) - snow_depth_out(:)))>0.000001) then
            call clm_repartition_snow(snow_depth_in(:))
          end if
        end if

        if (clmupdate_snow==2) then
          if ( SUM(ABS(h2osno_in(:) - h2osno_out(:)))>0.000001) then
      ...
      if ( clmupdate_snow_repartitioning==3) then

        if (clmupdate_snow==1) then
          do j=clm_begc,clm_endc
            if (snow_depth_out(j)>1.0d-6) then
                if (snow_depth_in(j)>1.0d-6) then
        ...
        if (clmupdate_snow==2 .or. clmupdate_snow==3) then
          do j=clm_begc,clm_endc
            if (h2osno_out(j)>1.0d-6) then
                if (h2osno_in(j)>1.0d-6) then
                  ! Update h2osoi_ice with increment
        ...
        if (clmupdate_snow==4 .or. clmupdate_snow==5 .or. clmupdate_snow==6 .or. clmupdate_snow==7) then
          do j=clm_begc,clm_endc
            if (h2osno_out(j)>1.0d-6) then
              if (h2osno_in(j)>1.0d-6) then
                if (snow_depth_out(j)>1.0d-6) then
                  if (snow_depth_in(j)>1.0d-6) then
                    ...
                    do i=snlsno(j)+1,0
                      ...
                      if (clmupdate_snow==4                          .or. clmupdate_snow==6) then
                      ...
                      if (clmupdate_snow==4 .or. clmupdate_snow==5) then
...
!        ! Set existing layers to near zero and let CLM do the layer aggregation
        ...
        if (snlsno(jj)<0) then ! snow layers in the column
          if (clmupdate_snow == 1) then
            ...
            if (snowdp(jj)>0.0_r8) then
              rho_avg = min(800.0_r8, h2osno(jj)/snowdp(jj))
            else
              rho_avg = 200.0_r8
            end if
            if (frac_sno(jj)>0.0_r8 .and. snlsno(jj)<0.0_r8) then
          ...
          do ii=0,snlsno(jj)+1,-1 ! iterate through the snow layers
            ...
            if (dzsno(jj,ii)>0.0_r8) then
              snowden = (h2oliq(jj,ii) + h2oice(jj,ii) / dzsno(jj,ii))
            else
              snowden = 0.0_r8
            endif
            ! fraction of SWE in each active layers
            if(h2osno_pr(jj)>0.0_r8) then
              ! repartition Option 1: Adjust bottom layer only (set weight to 1 for bottom 0 else)
              if(clmupdate_snow_repartitioning==1) then
                if (ii == 0) then ! DART version indexing check against nlevsno but for us 0
                  frac_swe = 1.0_r8
              ...
              else if (clmupdate_snow_repartitioning==2) then
            ...
            if ((h2oliq(jj,ii) + h2oice(jj,ii))>0.0_r8) then
            ...
            if (abs(h2osno_po(jj) - h2osno_pr(jj))>0.0_r8) then
        ...
        else if (clmupdate_snow == 2 .or. clmupdate_snow == 3) then
          ! Update the total snow depth to match udpates to layers for active snow layers
          if (abs(h2osno_po(jj) - h2osno_pr(jj)) > 0.0_r8 .and. snlsno(jj) < 0.0_r8) then
